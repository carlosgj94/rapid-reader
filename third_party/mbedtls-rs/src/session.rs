...
impl<'a> SessionState<'a> {
    ...
    fn new(conf: &SessionConfig<'a>) -> Result<Self, MbedtlsError> {
        ...
        // Pin both min and max to the configured version. On the device client we set
        // `min_version = TLS 1.2` to avoid the substantially heavier TLS 1.3 client-hello
        // ephemeral key-exchange path, which is currently the failing allocation site.
        //
        // We write the raw fields directly for compatibility with the Espressif MbedTLS port.
        let tls_version = conf.min_version().mbed_tls_version();
        ssl_config.private_min_tls_version = tls_version;
        ssl_config.private_max_tls_version = tls_version;
