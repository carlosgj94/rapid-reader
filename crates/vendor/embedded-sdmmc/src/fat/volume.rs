    ...
    D::Error: core::fmt::Debug,
{
    ...
    match bpb.fat_type {
        ...
        FatType::Fat32 => {
            ...
            let info_sector = match InfoSector::create_from_bytes(info_block) {
                Ok(info) => Some(info),
                Err(err) => {
                    // Some cards/tools produce a valid FAT32 volume but leave
                    // FSInfo signatures invalid. For read-first bring-up we can
                    // continue with unknown free-cluster hints.
                    warn!("Invalid FAT32 FSInfo sector; continuing: {}", err);
                    None
                }
            };

            let free_clusters_count = info_sector.as_ref().and_then(InfoSector::free_clusters_count);
            let next_free_cluster = info_sector.as_ref().and_then(InfoSector::next_free_cluster);
